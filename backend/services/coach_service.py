...
TABAN_BEKLEME_SN = 3    # sohbet interaktif -> kisa bekleme (3sn, 6sn)
...
class CareerCoachService:
    ...
    def _gunluk_kota_mi(hata: Exception) -> bool:
        """
        429 iki farkli sey olabilir:
          - RPM (dakikalik) asimi -> birkac saniyede duzelir  -> RETRY ANLAMLI
    ...
    def chat(self, message: str, session_id: str | None = None) -> tuple[str, str]:
        ...
        # --- Ustel geri cekilme (learning_service.py ile ayni desen) ---
        for deneme in range(MAX_DENEME):
            try:
                ...
                # Hafizaya SADECE basarili cevapta yaz (yarim tur birakma)
                session["history"].append(("user", message))
                session["history"].append(("model", reply))
                return reply, sid
                ...
                if not gecici:
                    print(f"[CareerCoachService] API hatasi {kod}: {e}")
                    raise

                if kod == 429 and self._gunluk_kota_mi(e):
                    print("[CareerCoachService] GUNLUK KOTA DOLDU. "
                          "Kota Pasifik saatiyle gece yarisi sifirlanir.")
                    raise

                if deneme < MAX_DENEME - 1:
                    bekle = TABAN_BEKLEME_SN * (2 ** deneme) + random.uniform(0, 1)
                    tur = "Rate limit (RPM)" if kod == 429 else f"Sunucu hatasi ({kod})"
                    print(f"[CareerCoachService] {tur}, {bekle:.1f} sn bekleniyor... "
                          f"(deneme {deneme + 1}/{MAX_DENEME})")
                    time.sleep(bekle)
                    continue

                print(f"[CareerCoachService] {MAX_DENEME} deneme sonrasi basarisiz.")
                raise
