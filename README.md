...
- **Tahmini puan tamamlama tablosu (Sprint 3):**
  ...
  | Koç kalite eval'i: 6 prob (bağlam sadakati, sayı temellendirmesi, hafıza, injection) → 6/6 + çok koşulu ölçüm için `--tekrar N` | 5 | ✅ Done |
...
*   **Ölçülmüş Koç Güvenliği:** Prompt injection direnci ve bağlam sadakati göz kararıyla değil eval'le doğrulandı. Sayı temellendirmesi kontrolü, koçun bağlamda olmayan bir yüzde uydurmadığını kanıtladı (6/6). Tek koşunun `temperature=0.6` altında şans olabileceği fark edildi ve bataryayı N kez koşup prob bazında kararlılık raporlayan `--tekrar` seçeneği eklendi.
...
Ölçüm: **6/6 prob geçti (%100, tek koşu)**. En kritik kontrol `sayi_grounding` — koçun
"%88 eşleşen ilan" derken bu sayıyı gerçekten bağlamdan alıp almadığını denetler.
Sonuç: **uydurma yüzde yok.**

Koç `temperature=0.6` ile çalıştığı için tek koşu şans olabilir; bataryayı N kez koşan
`--tekrar N` seçeneği mevcuttur (`python -m evals.coach.quality --tekrar 3`, 18 çağrı).
...
| Koç kalitesi | `coach/quality` | **6/6 (%100)** — tek koşu |
